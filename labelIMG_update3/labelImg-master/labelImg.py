...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def loadLabels(self, shapes):
        s = []
        for label, points, line_color, fill_color, difficult, owner in shapes:
            shape = Shape(label=label)
            shape.setOwner(owner)
            for x, y in points:
    ...
    def openFile(self, _value=False):
        ...
        # msg_box = QMessageBox(QMessageBox.Warning, "Alert", filters)
        # msg_box.exec_()
    ...
    def loadPascalXMLByFilename(self, xmlPath):
        ...
        if os.path.exists(xmlPath+'.xml'):
            ...
            xmllist=glob.glob(str(xmlPath+'*.xml'));
        for ifile in xmllist:
            print (ifile)
            if os.path.isfile(ifile) is False:
                return
            tVocParseReader = PascalVocReader(ifile,own)
            for item in tVocParseReader.getShapes():
                print(item)
                shapes.append(item)
            own=own+1
            
        # msg_box = QMessageBox(QMessageBox.Warning, "Alert", str(shapes))
        # msg_box.exec_()
